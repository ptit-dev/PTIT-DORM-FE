...
const ManageEmployee: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      showNotification("Lỗi nhập liệu", errorMessages, "error");
      setSubmitting(false);
      return;
    }
    ...
    try {
      if (editId) {
        ...
        showNotification("Thành công", "Cập nhật thông tin nhân viên thành công", "success");
      } else {
        await createManager(data);
        showNotification("Thành công", "Thêm nhân viên mới thành công", "success");
      }
      setModalOpen(false);
      fetchManagers();
    } catch (e: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showNotification("Thành công", "Xóa nhân viên thành công", "success");
      setDeleteId(null);
