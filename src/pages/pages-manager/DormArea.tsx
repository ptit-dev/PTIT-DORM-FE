...
const DormArea: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (modal.data && modal.data.id) {
        ...
        showNotification("Thành công", "Cập nhật khu KTX thành công", "success");
      } else {
        await createDormArea(data);
        showNotification("Thành công", "Thêm mới khu KTX thành công", "success");
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showNotification("Thành công", "Xóa khu KTX thành công", "success");
      setDeleteId(null);
      fetchAreas();
    } catch (err) {
      if (err instanceof Error) {
        showNotification("Lỗi xóa", err.message || "Lỗi xóa khu KTX", "error");
      } else {
